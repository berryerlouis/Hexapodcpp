...
namespace Service
{
    namespace Display
    {
        ServiceDisplay::ServiceDisplay(
            ...
            : Service(DISPLAY, 10U, messageListener, eventDispatcher)
            , mSsd1306(ssd1306)
            , mBmpBatteryLevel{.bmp = const_cast<uint8_t *>(Bitmaps::Battery0),
                               .width = 16U,
                               .height = 7U}
            , mBmpCommunication{.bmp = const_cast<uint8_t *>(
                                        Bitmaps::Communication),
                                .width = 16U,
                                .height = 8U}
        ...
        void ServiceDisplay::Update(const uint64_t currentTime) {
            ...
            if ((this->mNotifiedTimeProximityLaser > 0U) &&
                ...
                this->DisplayProximitySensor(
                        Component::Proximity::SensorsId::VLX, 0xFFFFU);
            }
            if ((this->mNotifiedTimeProximityUsLeft > 0U) &&
                ((currentTime - this->mNotifiedTimeProximityUsLeft) >=
                 1000UL)) {
                this->DisplayProximitySensor(
                        Component::Proximity::SensorsId::SRF_LEFT, 0xFFFFU);
            }
            if ((this->mNotifiedTimeProximityUsRight > 0U) &&
                ((currentTime - this->mNotifiedTimeProximityUsRight) >=
                 1000UL)) {
                this->DisplayProximitySensor(
                        Component::Proximity::SensorsId::SRF_RIGHT, 0xFFFFU);
            }
        ...
        void ServiceDisplay::DisplayBackground(void) const {
            ...
            this->mSsd1306.DrawLine(
                    18U, 10U, 18U, SCREEN_HEIGHT, Bitmaps::Color::COLOR_WHITE);
        ...
        void ServiceDisplay::DisplayButtonBmp(
                const Button::ButtonState &buttonState) {
            this->mSsd1306.EraseArea(SCREEN_WIDTH -
                                             this->mBmpCommunication.width -
                                             2U - this->mBmpButton.width,
                                     0U,
                                     this->mBmpButton.width,
                                     8U);
            if (buttonState == Button::ButtonState::RELEASE) {
                this->mBmpButton.bmp =
                        const_cast<uint8_t *>(Bitmaps::ButtonRelease);
            } else {
                this->mBmpButton.bmp =
                        const_cast<uint8_t *>(Bitmaps::ButtonPush);
            }
        ...
        void
        ServiceDisplay::DisplayBatteryLevel(const Battery::BatteryState state) {
            if (state == Battery::BatteryState::WARNING) {
                this->mBmpBatteryLevel.bmp =
                        const_cast<uint8_t *>(Bitmaps::Battery50);
            } else if (state == Battery::BatteryState::NOMINAL) {
                this->mBmpBatteryLevel.bmp =
                        const_cast<uint8_t *>(Bitmaps::Battery100);
            } else {
                this->mBmpBatteryLevel.bmp =
                        const_cast<uint8_t *>(Bitmaps::Usb);
            }
            this->mSsd1306.DrawBitmap(
                    &this->mBmpBatteryLevel, 0, 0, Bitmaps::Color::COLOR_WHITE);
        }
        ...
        void ServiceDisplay::DisplayProximitySensor(
            ...
            if (sensorId == Component::Proximity::SensorsId::SRF_LEFT) {
                if (distance > 30U) {
                    ...
                    this->mNotifiedTimeProximityUsLeft =
                            Timer::Tick::GetInstance().GetMs();
                    this->mBmpProximity.bmp =
                            const_cast<uint8_t *>(Bitmaps::ArrowLeft);
                ...
                if (distance > 300U) {
                    ...
                    this->mNotifiedTimeProximityLaser =
                            Timer::Tick::GetInstance().GetMs();
                    this->mBmpProximity.bmp =
                            const_cast<uint8_t *>(Bitmaps::ArrowUp);
                    this->mSsd1306.DrawBitmap(
                            &this->mBmpProximity,
                            (SCREEN_WIDTH / 2U) -
                                    (this->mBmpProximity.width / 2U),
                ...
                if (distance > 30U) {
                    ...
                    this->mNotifiedTimeProximityUsRight =
                            Timer::Tick::GetInstance().GetMs();
                    this->mBmpProximity.bmp =
                            const_cast<uint8_t *>(Bitmaps::ArrowRight);
                    this->mSsd1306.DrawBitmap(
                            &this->mBmpProximity,
                            (SCREEN_WIDTH - this->mBmpProximity.width),
                            SCREEN_HEIGHT - 10U,
        ...
        void ServiceDisplay::DisplaySound(const SoundStruct &soundStruct) {
            if (soundStruct.id == SOUND_RIGHT) {
                if (soundStruct.delay > 0U) {
                    this->mNotifiedTimeSoundRight =
                            Timer::Tick::GetInstance().GetMs();
                    this->mBmpSound.bmp =
                            const_cast<uint8_t *>(Bitmaps::SoundRight);
                ...
                if (soundStruct.delay > 0U) {
                    this->mNotifiedTimeSoundLeft =
                            Timer::Tick::GetInstance().GetMs();
                    this->mBmpSound.bmp =
                            const_cast<uint8_t *>(Bitmaps::SoundLeft);
                    this->mSsd1306.DrawBitmap(&this->mBmpSound,
                                              0U,
                                              12U,
                                              Bitmaps::Color::COLOR_WHITE);
                    ...
                    this->mSsd1306.EraseArea(
                            0U, 12U, this->mBmpSound.width, 8U);
                }
            } else {
                this->mNotifiedTimeSoundRight = 0U;
                this->mNotifiedTimeSoundLeft = 0U;
        ...
        void ServiceDisplay::OnEvent(const Event::Event &event) {
            if (event.serviceId == EServices::COMMUNICATION) {
                if (event.eventType == EventType::EVENT_COM_UPDATE) {
                    if (event.eventArg.type() == typeid(CommunicationStruct)) {
                        const CommunicationStruct comStruct =
                                std::any_cast<CommunicationStruct>(
                                        event.eventArg);
                ...
                if (event.eventType == EventType::EVENT_BATTERY_UPDATE) {
                    if (event.eventArg.type() ==
                        typeid(Component::Battery::BatteryStruct)) {
                        const Component::Battery::BatteryStruct batteryStruct =
                                std::any_cast<
                                        Component::Battery::BatteryStruct>(
                                        event.eventArg);
                ...
                if (event.eventType == EventType::EVENT_SENSOR_UPDATE) {
                    if (event.eventArg.type() ==
                        typeid(Component::Proximity::SensorsStruct)) {
                        const Component::Proximity::SensorsStruct sensorStruct =
                                std::any_cast<
                                        Component::Proximity::SensorsStruct>(
                                        event.eventArg);
                        this->DisplayProximitySensor(sensorStruct.id,
                                                     sensorStruct.distance);
                    }
                }
            } else if (event.serviceId == EServices::SOUND) {
                if (event.eventType == EventType::EVENT_SOUND_UPDATE) {
                    if (event.eventArg.type() ==
                        typeid(Component::Sound::SoundStruct)) {
                        const Component::Sound::SoundStruct soundStruct =
                                std::any_cast<Component::Sound::SoundStruct>(
                                        event.eventArg);
