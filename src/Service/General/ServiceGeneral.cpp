...
namespace Service
{
    namespace General
    {
        ServiceGeneral::ServiceGeneral(LedPwmInterface                 &ledPwm,
        ...
        Core::Status ServiceGeneral::Initialize() {
            ...
            if ((Core::Status::CORE_OK == success1) && (Core::Status::CORE_OK == success2)) {
        ...
        void ServiceGeneral::OnEvent(const Event::Event &event) {
            if (event.eventType == EventType::EVENT_INIT_UPDATE &&
                ...
                if (std::any_cast<Core::Status>(event.eventArg) != Core::Status::CORE_OK) {
                    LOG_SERVICE_ERROR("Service Id %s(%d) Failed on "
                                      "initialization.",
