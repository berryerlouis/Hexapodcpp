...
namespace Cluster
{
    namespace Battery
    {
        ...
        Core::Status ClusterBattery::BuildFrameVoltage(const uint16_t voltage,
                                                       Frame         &response) {
            const Core::Status success = response.Build(EClusters::BATTERY, EBatteryCommands::GET_VOLTAGE);
        ...
        Core::Status ClusterBattery::BuildFrameIntensity(const uint16_t intensity,
                                                         Frame         &response) {
            const Core::Status success = response.Build(EClusters::BATTERY, EBatteryCommands::GET_CURRENT);
            if (success == Core::Status::CORE_OK) {
        ...
        Core::Status ClusterBattery::BuildFrameState(const uint16_t state,
                                                     ...
                                                     Frame         &response) {
            const Core::Status success = response.Build(EClusters::BATTERY, EBatteryCommands::GET_BAT_STATUS);
            if (success == Core::Status::CORE_OK) {
