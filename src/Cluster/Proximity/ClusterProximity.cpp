...
namespace Cluster
{
    namespace Proximity
    {
        ...
        Core::Status ClusterProximity::ExecuteFrame(const Frame &request,
                                                    Frame       &response) {
        ...
        Core::Status
        ClusterProximity::BuildFrameDistance(const SensorsId sensorId,
                                             const uint16_t  distance,
                                             Frame          &response) {
            const Core::Status success =
                    response.Build(EClusters::PROXIMITY, sensorId);
        ...
        Core::Status
        ClusterProximity::BuildFrameThreshold(const SensorsId sensorId,
                                              const uint16_t  threshold,
                                              Frame          &response) {
            const Core::Status success = response.Build(
                    EClusters::PROXIMITY, EProximityCommands::SET_THRESHOLD);
            if (success == Core::Status::CORE_OK) {
