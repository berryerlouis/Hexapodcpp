...
namespace Cluster
{
    namespace Servo
    {
        ...
        Core::Status ClusterServo::ExecuteFrame(const Frame &request,
            ...
            if (request.GetCommandId() == EServoCommands::SET_ANGLE) {
                ...
                const Core::Status success =
                        this->mServosInterface.GetServo(static_cast<EServos>(servoId))->SetAngle(angle);
                if (success == Core::Status::CORE_OK) {
                    const uint8_t angleServo =
                            this->mServosInterface.GetServo(static_cast<EServos>(servoId))->GetAngle();
            ...
            if (request.GetCommandId() == EServoCommands::SET_MIN) {
                ...
                const uint8_t      angle = request.Get1ByteParam(1U);
                const Core::Status success =
                        this->mServosInterface.GetServo(static_cast<EServos>(servoId))->SetMin(angle)
                                ? Core::CORE_OK
                                : Core::CORE_ERROR_MIN;
                if (success == Core::Status::CORE_OK) {
            ...
            if (request.GetCommandId() == EServoCommands::SET_MAX) {
                ...
                const uint8_t      angle = request.Get1ByteParam(1U);
                const Core::Status success =
                        this->mServosInterface.GetServo(static_cast<EServos>(servoId))->SetMax(angle)
                                ? Core::CORE_OK
                                : Core::CORE_ERROR_MAX;
                if (success == Core::Status::CORE_OK) {
            ...
            if (request.GetCommandId() == EServoCommands::SET_OFFSET) {
                ...
                const Core::Status success =
                        this->mServosInterface.GetServo(static_cast<EServos>(servoId))->SetOffset(angle)
                                ? Core::CORE_OK
                                : Core::CORE_ERROR_MAX;
                if (success == Core::Status::CORE_OK) {
        ...
        Core::Status ClusterServo::BuildFrameGetAngle(const uint8_t servoId,
                                                      const uint8_t angle,
                                                      Frame        &response) {
            const Core::Status success = response.Build(EClusters::SERVO, EServoCommands::GET_ANGLE);
            if (success == Core::Status::CORE_OK) {
        ...
        Core::Status ClusterServo::BuildFrameSetAngle(const uint8_t servoId,
                                                      const uint8_t angle,
                                                      Frame        &response) {
            const Core::Status success = response.Build(EClusters::SERVO, EServoCommands::SET_ANGLE);
            if (success == Core::Status::CORE_OK) {
                response.Set1ByteParam(servoId);
                response.Set1ByteParam(angle);
            }
        ...
        Core::Status ClusterServo::BuildFrameGetOffset(const uint8_t servoId,
                                                       const uint8_t angle,
                                                       Frame        &response) {
            const Core::Status success = response.Build(EClusters::SERVO, EServoCommands::GET_OFFSET);
            if (success == Core::Status::CORE_OK) {
                response.Set1ByteParam(servoId);
                response.Set1ByteParam(angle);
            }
        ...
        Core::Status ClusterServo::BuildFrameSetOffset(const uint8_t servoId,
                                                       const uint8_t angle,
                                                       Frame        &response) {
            const Core::Status success = response.Build(EClusters::SERVO, EServoCommands::SET_OFFSET);
            if (success == Core::Status::CORE_OK) {
                response.Set1ByteParam(servoId);
                response.Set1ByteParam(angle);
            }
        ...
        Core::Status ClusterServo::BuildFrameGetState(const uint8_t servoId,
                                                      const bool    state,
                                                      Frame        &response) {
            const Core::Status success = response.Build(EClusters::SERVO, EServoCommands::GET_STATE);
            if (success == Core::Status::CORE_OK) {
                response.Set1ByteParam(servoId);
        ...
        Core::Status ClusterServo::BuildFrameSetState(const uint8_t servoId,
                                                      const bool    state,
                                                      Frame        &response) {
            const Core::Status success = response.Build(EClusters::SERVO, EServoCommands::SET_STATE);
            if (success == Core::Status::CORE_OK) {
                response.Set1ByteParam(servoId);
                response.Set1ByteParam(state);
            }
        ...
        Core::Status ClusterServo::BuildFrameGetReverse(const uint8_t servoId,
                                                        const bool    reversed,
                                                        Frame        &response) {
            const Core::Status success = response.Build(EClusters::SERVO, EServoCommands::GET_REVERSE);
            if (success == Core::Status::CORE_OK) {
                response.Set1ByteParam(servoId);
        ...
        Core::Status ClusterServo::BuildFrameSetReverse(const uint8_t servoId,
                                                        const bool    reversed,
                                                        Frame        &response) {
            const Core::Status success = response.Build(EClusters::SERVO, EServoCommands::SET_REVERSE);
            if (success == Core::Status::CORE_OK) {
                response.Set1ByteParam(servoId);
                response.Set1ByteParam(reversed);
            }
        ...
        Core::Status ClusterServo::BuildFrameGetStatePca(const bool state,
                                                         Frame     &response) {
            const Core::Status success = response.Build(EClusters::SERVO, EServoCommands::GET_STATE_PCA);
            if (success == Core::Status::CORE_OK) {
                response.Set1ByteParam(state);
            }
            return (success);
        ...
        Core::Status ClusterServo::BuildFrameSetError(const Core::Status error,
                                                      ...
                                                      const uint8_t      angle,
                                                      Frame             &response) {
            const Core::Status success = response.Build(EClusters::SERVO, EClusterCommandGeneric::GENERIC);
            if (success == Core::Status::CORE_OK) {
                response.Set1ByteParam(servoId);
