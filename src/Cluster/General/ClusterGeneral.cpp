    ...
    namespace General
    {
        ...
        Core::Status ClusterGeneral::ExecuteFrame(const Frame &request,
            ...
            if (request.GetCommandId() == EGeneralCommands::RESET) {
                ...
                success =
                        this->BuildFrameGetMinTime(Service::EServices::GENERAL, this->mSoftware.GetMinTime(), response);
            } else if (request.GetCommandId() == EGeneralCommands::MAX_EXECUTION_TIME) {
                success =
                        this->BuildFrameGetMaxTime(Service::EServices::GENERAL, this->mSoftware.GetMaxTime(), response);
        ...
        Core::Status ClusterGeneral::BuildFrameReset(Frame             &response,
            ...
            const Core::Status success = response.Build(EClusters::GENERAL, EGeneralCommands::RESET);
        ...
        Core::Status ClusterGeneral::BuildFrameGetVersion(const SoftwareInterface::Version version,
                                                          Frame                           &response) {
            const Core::Status success = response.Build(EClusters::GENERAL, EGeneralCommands::VERSION);
            if (success == Core::Status::CORE_OK) {
        ...
        Core::Status ClusterGeneral::BuildFrameGetMinTime(const uint8_t  serviceId,
                                                          ...
                                                          Frame         &response) {
            const Core::Status success = response.Build(EClusters::GENERAL, EGeneralCommands::MIN_EXECUTION_TIME);
            if (success == Core::Status::CORE_OK) {
        ...
        Core::Status ClusterGeneral::BuildFrameGetMaxTime(const uint8_t  serviceId,
                                                          const uint64_t deltaTime,
                                                          Frame         &response) {
            const Core::Status success = response.Build(EClusters::GENERAL, EGeneralCommands::MAX_EXECUTION_TIME);
            if (success == Core::Status::CORE_OK) {
                response.Set1ByteParam(serviceId);
                response.Set8BytesParam(deltaTime);
            }
        ...
        Core::Status ClusterGeneral::BuildFrameResetTime(const uint8_t serviceId,
                                                         Frame        &response) {
            const Core::Status success = response.Build(EClusters::GENERAL, EGeneralCommands::RESET_EXECUTION_TIME);
            if (success == Core::Status::CORE_OK) {
                response.Set1ByteParam(serviceId);
            }
            return (success);
