...
namespace Cluster
{
    namespace Body
    {
        ...
        Core::Status ClusterBody::ExecuteFrame(const Frame &request,
            ...
            if (request.GetCommandId() == EBodyCommands::GET_ALL_PARAMS) {
                ...
                const Position3d position = {
                    .x = request.Get2BytesParam(0U) / 10.0f, .y = request.Get2BytesParam(2U) / 10.0f,
                    .z = request.Get2BytesParam(4U) / 10.0f
                };
                const Rotation3d rotation = {
                    .angleX = request.Get2BytesParam(6U) / 10.0f, .angleY = request.Get2BytesParam(8U) / 10.0f,
                    .angleZ = request.Get2BytesParam(10U) / 10.0f
                };
                const uint16_t travelTime = request.Get2BytesParam(12U);
                const uint32_t successMove = this->mBody.SetBodyPositionRotation(position,
                    rotation,
                    travelTime);
                success = this->BuildFrameSetBodyPosition(response,
                                                          successMove);
            } else if (request.GetCommandId() == EBodyCommands::SET_LEG_POS_ROT) {
                const uint8_t legId = request.Get1ByteParam(0U);
                const Position3d position = {
                    .x = request.Get2BytesParam(1U) / 10.0f, .y = request.Get2BytesParam(3U) / 10.0f,
                    .z = request.Get2BytesParam(5U) / 10.0f
                };
                const uint16_t travelTime = request.Get2BytesParam(7U);
                const uint32_t successMove = this->mBody.SetLegPositionRotation(legId,
                    position,
                    travelTime);
                success = this->BuildFrameSetLegPosition(response,
                                                         successMove);
                ...
                success = this->BuildFrameUpdateDirection(response,
                ...
                success = this->BuildFrameUpdateAmplitude(response,
                                                          this->mBody.GetAmplitude() * 10U);
                ...
                success = this->BuildFrameUpdateElevation(response,
                                                          this->mBody.GetElevation() * 10U);
                ...
                success = this->BuildFrameUpdateDuration(response,
                ...
                success = this->BuildFrameUpdateGait(response,
                                                     this->mBody.GetGait());
        ...
        Core::Status ClusterBody::BuildFrameSetBodyPosition(Frame &response,
            ...
            const Core::Status success = response.Build(EClusters::BODY,
