...
namespace Cluster
{
    namespace Body
    {
        ...
        Core::Status ClusterBody::ExecuteFrame(const Frame &request,
                                               Frame       &response) {
            ...
            if (request.GetCommandId() == EBodyCommands::GET_ALL_PARAMS) {
                ...
                const Move::Walk::EWalkStatus status =
                        static_cast<Move::Walk::EWalkStatus>(
                ...
                const Move::Gait::GaitType gait =
                        static_cast<Move::Gait::GaitType>(
                                request.Get1ByteParam(0U));
        ...
        Core::Status
        ClusterBody::BuildFrameSetBodyPosition(Frame         &response,
                                               const uint32_t successMove) {
            const Core::Status success = response.Build(
                    EClusters::BODY, EBodyCommands::SET_BODY_POS_ROT);
        ...
        Core::Status
        ClusterBody::BuildFrameSetLegPosition(Frame         &response,
                                              const uint32_t successMove) {
            const Core::Status success = response.Build(
                    EClusters::BODY, EBodyCommands::SET_LEG_POS_ROT);
            if (success == Core::Status::CORE_OK) {
                response.Set4BytesParam(successMove);
            }
            return success;
        ...
        Core::Status
        ClusterBody::BuildFrameUpdateWalkStatus(Frame        &response,
                                                const uint8_t walkStatus) {
            const Core::Status success = response.Build(
                    EClusters::BODY, EBodyCommands::SET_WALK_STATUS);
            if (success == Core::Status::CORE_OK) {
        ...
        Core::Status
        ClusterBody::BuildFrameUpdateDirection(Frame         &response,
                                               const uint16_t direction) {
            const Core::Status success = response.Build(
                    EClusters::BODY, EBodyCommands::GET_DIRECTION);
            if (success == Core::Status::CORE_OK) {
        ...
        Core::Status
        ClusterBody::BuildFrameUpdateRotation(Frame         &response,
                                              const uint16_t rotation,
        ...
        Core::Status
        ClusterBody::BuildFrameUpdateAmplitude(Frame        &response,
                                               const uint8_t amplitude) {
            const Core::Status success = response.Build(
                    EClusters::BODY, EBodyCommands::GET_AMPLITUDE);
            if (success == Core::Status::CORE_OK) {
        ...
        Core::Status
        ClusterBody::BuildFrameUpdateElevation(Frame        &response,
                                               const uint8_t elevation) {
            const Core::Status success = response.Build(
                    EClusters::BODY, EBodyCommands::GET_ELEVATION);
            if (success == Core::Status::CORE_OK) {
        ...
        Core::Status ClusterBody::BuildFrameUpdateAll(Frame         &response,
            ...
            const Core::Status success = response.Build(
                    EClusters::BODY, EBodyCommands::GET_ALL_PARAMS);
            if (success == Core::Status::CORE_OK) {
                response.Set1ByteParam(amplitude);
                response.Set1ByteParam(elevation);
                response.Set2BytesParam(direction);
        ...
        Core::Status
        ClusterBody::BuildFrameUpdateDuration(Frame         &response,
                                              const uint16_t duration) {
            const Core::Status success = response.Build(
                    EClusters::BODY, EBodyCommands::GET_DURATION);
            if (success == Core::Status::CORE_OK) {
                response.Set2BytesParam(duration);
            }
            return success;
        ...
        Core::Status
        ClusterBody::BuildFrameUpdateGait(Frame                     &response,
                                          const Move::Gait::GaitType gait) {
            const Core::Status success =
                    response.Build(EClusters::BODY, EBodyCommands::GET_GAIT);
            if (success == Core::Status::CORE_OK) {
