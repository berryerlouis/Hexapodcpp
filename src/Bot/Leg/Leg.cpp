...
namespace Bot
{
    namespace Leg
    {
        ...
        void Leg::ComputeRotation(Position3d &position,
            ...
            this->ComputeDirection(position,
                                   0U);
        }
        ...
        void Leg::ComputeLerpTarget(const uint64_t currentTime,
            ...
            if (false == isRotated) {
                ...
                this->ComputeRotation(position,
                                      rotation,
                                      clockWize);
            }
            this->ComputeAmplitude(position,
                                   amplitude);
            this->ComputeElevation(position,
                                   elevation);
            ...
            if (timeDiff < 500U) {
                ...
                position = Misc::Utils::LerpPosition(this->mStartPos,
                                                     position,
        ...
        Core::Status Leg::SetLegIk(const Position3d &position,
            ...
            this->mLegIk.coxaIk = 90.0F + atan2(this->mLegIk.newFootPos.y,
                                                this->mLegIk.newFootPos.x) * 180.0F / M_PI;

            // Normalize angle: first wrap to -180 to +180 range
            this->mLegIk.coxaIk = fmod(this->mLegIk.coxaIk + 180.0F,
                                       360.0F) - 180.0F;
            ...
            success |= this->mCoxa.SetAngle(static_cast<uint8_t>(this->mLegIk.coxaIk),
                                            travelTime);
            success |= this->mFemur.SetAngle(static_cast<uint8_t>(this->mLegIk.femurIk),
                                             travelTime);
            success |= this->mTibia.SetAngle(static_cast<uint8_t>(this->mLegIk.tibiaIk),
                                             travelTime);
