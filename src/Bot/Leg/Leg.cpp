...
namespace Bot
{
    namespace Leg
    {
        Leg::Leg(const Legs::ELeg legId,
            ...
            switch (mLegId) {
                case Legs::ELeg::FRONT_LEFT:
                    this->mBodyCenterOffsetX =
                            -BODY_LEG_FRONT_REAR_FROM_CENTER_X_LENGTH;
                    this->mBodyCenterOffsetY =
                            BODY_LEG_FRONT_REAR_FROM_CENTER_Y_LENGTH;

                    this->mFootPosition.x = -cos(Misc::Utils::ToRad(60.0F)) *
                                            (COXA_LENGTH + FEMUR_LENGTH);
                    this->mFootPosition.y = sin(Misc::Utils::ToRad(60.0F)) *
                                            (COXA_LENGTH + FEMUR_LENGTH);
                ...
                case Legs::ELeg::MIDDLE_LEFT:
                    this->mBodyCenterOffsetX =
                            -BODY_LEG_MIDDLE_FROM_CENTER_X_LENGTH;
                    this->mBodyCenterOffsetY =
                            BODY_LEG_MIDDLE_FROM_CENTER_Y_LENGTH;
        ...
        void Leg::ComputeDirection(Position3d &position,
            ...
            const float newAngle = (this->GetId() < Legs::NB_LEGS / 2U)
                                           ? angleDirection
                                           : -angleDirection - M_PI;

            rotatePosition.x = (position.x * cos(newAngle)) +
                               (position.y * clockwize * sin(newAngle));
            rotatePosition.y = (position.y * cos(newAngle)) -
                               (position.x * clockwize * sin(newAngle));
        ...
        Core::Status Leg::SetLegIk(const Position3d &position,
            ...
            this->mLegIk.newFootPos.x =
                    position.x + (COXA_LENGTH + FEMUR_LENGTH);
            this->mLegIk.newFootPos.y = position.y + 0;
            this->mLegIk.newFootPos.z = position.z + TIBIA_LENGTH;

            // compute angles from distances
            this->mLegIk.coxaFootDist =
                    sqrt(this->mLegIk.newFootPos.x * this->mLegIk.newFootPos.x +
            ...
            this->mLegIk.ika1 = atan((this->mLegIk.coxaFootDist - COXA_LENGTH) /
                                     this->mLegIk.newFootPos.z);
            this->mLegIk.ika2 =
                    acos(((TIBIA_LENGTH * TIBIA_LENGTH) -
                          (FEMUR_LENGTH * FEMUR_LENGTH) -
                          (this->mLegIk.iksw * this->mLegIk.iksw)) /
                         (-2.0F * this->mLegIk.iksw * FEMUR_LENGTH));
            this->mLegIk.tangle =
                    acos(((this->mLegIk.iksw * this->mLegIk.iksw) -
                          (TIBIA_LENGTH * TIBIA_LENGTH) -
                          (FEMUR_LENGTH * FEMUR_LENGTH)) /
                         (-2.0F * FEMUR_LENGTH * TIBIA_LENGTH));
            this->mLegIk.tibiaIk =
                    90.0F + (90.0F - this->mLegIk.tangle * 180.0F / M_PI);
            this->mLegIk.femurIk =
                    90.0F + (90.0F - (this->mLegIk.ika1 + this->mLegIk.ika2) *
                                             180.0F / M_PI);
            this->mLegIk.coxaIk = 90.0F + atan2(this->mLegIk.newFootPos.y,
                                                this->mLegIk.newFootPos.x) *
                                                  180.0F / M_PI;

            // Normalize angle: first wrap to -180 to +180 range
            this->mLegIk.coxaIk =
                    fmod(this->mLegIk.coxaIk + 180.0F, 360.0F) - 180.0F;
            if (this->mLegIk.coxaIk < 0.0F) {
                this->mLegIk.coxaIk += 360.0F;
            }

            Core::Status success = Core::Status::CORE_OK;
            success |= this->mCoxa.SetAngle(
                    static_cast<uint8_t>(this->mLegIk.coxaIk), travelTime);
            success |= this->mFemur.SetAngle(
                    static_cast<uint8_t>(this->mLegIk.femurIk), travelTime);
            success |= this->mTibia.SetAngle(
                    static_cast<uint8_t>(this->mLegIk.tibiaIk), travelTime);
        ...
        Core::Status Leg::SetLegBodyIk(const Position3d &position,
                                       ...
                                       const uint16_t    travelTime) {
            this->mLegIk.newFootPos.x =
                    this->mFootPosition.x + position.x + bodyIk.x;
            this->mLegIk.newFootPos.y =
                    this->mFootPosition.y + position.y + bodyIk.y;
            this->mLegIk.newFootPos.z =
                    this->mFootPosition.z + position.z + bodyIk.z;
            this->mLegIk.coxaFootDist =
                    sqrt(this->mLegIk.newFootPos.x * this->mLegIk.newFootPos.x +
                         this->mLegIk.newFootPos.y * this->mLegIk.newFootPos.y);
            this->mLegIk.iksw = sqrt(
            ...
            this->mLegIk.femurIk =
                    90.0F + (90.0F - (this->mLegIk.ika1 + this->mLegIk.ika2) *
                                             180.0F / M_PI);
            this->mLegIk.coxaIk = atan2(this->mLegIk.newFootPos.y,
                                        this->mLegIk.newFootPos.x) *
                                  180.0F / M_PI;
            ...
            if (success != Core::Status::CORE_OK) {
                LOG_BOT_ERROR(
                        "Leg", "leg Id:%d Set Body IK error", this->mLegId);
            }
            return success;
        }
    } // namespace Leg
