...
namespace Component
{
    namespace Barometer
    {
        ...
        Core::Status Barometer::Initialize(void) {
            this->ReadProm();
            LOG_COMPONENT_DEBUG(
                    "Barometer", "address 0x%02X Initialized.", this->mAddress);
            return Core::CORE_OK;
        }
        ...
        void Barometer::Update(const uint64_t currentTime) {
            ...
            if (currentTime > lastTime + 10000U) {
                ...
                if (toggleReadTempPressure == false) {
                    ...
                    this->mI2c.SendCommand(this->mAddress,
                                           MS5611_CMD_CONV_D1 +
                                                   this->mResolution);
                } else {
                ...
                const int32_t deltaTime =
                        this->mRawTemp -
                        (static_cast<uint32_t>(this->mProm[4U]) * 256);
                const int32_t temp =
                        2000 +
                        (deltaTime * static_cast<int64_t>(this->mProm[5U])) /
                                8388608.0F;
                this->mTemp = temp;

                int64_t offset =
                        (static_cast<int64_t>(this->mProm[1U]) * 65536) +
                        (static_cast<int64_t>(this->mProm[3U]) * deltaTime) /
                                128.0F;
                int64_t sensibility =
                        (static_cast<int64_t>(this->mProm[0U]) * 32768) +
                        (static_cast<int64_t>(this->mProm[2U]) * deltaTime) /
                                256.0F;
                int64_t offset2 = 0;
                ...
                if (temp < -1500) {
                    ...
                    sensibility2 = sensibility2 +
                                   11 * ((temp + 1500) * (temp + 1500)) / 2.0F;
                }

                offset -= offset2;
                sensibility -= sensibility2;
                this->mPressure =
                        (this->mRawPressure * sensibility / 2097152.0F -
                         offset) /
                        32768.0F;
            }
        }
        ...
        void Barometer::ReadProm(void) {
            for (size_t offset = 0U; offset < 6U; offset++) {
                uint8_t data[2U] = {0U};
                this->mI2c.ReadRegisters(this->mAddress,
                                         MS5611_CMD_READ_PROM + (offset * 2U),
                                         data,
                                         2U);
        ...
        uint32_t Barometer::ReadRawTemp(void) const {
            ...
            this->mI2c.ReadRegisters(
                    this->mAddress, MS5611_CMD_ADC_READ, data, 3U);
