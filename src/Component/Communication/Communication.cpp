...
namespace Component
{
    namespace Communication
    {
        ...
        void Communication::Notified(const Socket::SocketStruct &state) {
            this->Notify(state == Socket::SocketStruct::CLIENT_CONNECTED
                                 ? CLIENT_CONNECTED
                                 : NO_CLIENT);
        }
        ...
        void Communication::Update(const uint64_t currentTime) {
            ...
            if (true == this->ReceivedStringFrame()) {
                ...
                const Core::Status parsedStatus = Protocol::Decode(
                        const_cast<const char *>(this->mBufferRx), request);
                    ...
                    if (frameClusterID < NB_CLUSTERS) {
                        ...
                        if (cluster != nullptr) {
                            if (cluster->Execute(request, response) !=
                                ...
                                response.Set1ByteParam(
                                        Core::Status::CORE_ERROR_ARGUMENT);
                            ...
                            response.Build(frameClusterID, GENERIC);
                            response.Set1ByteParam(
                                    Core::Status::CORE_ERROR_NULLPTR);
                            LOG_COMPONENT_ERROR("Communication",
                                                "RX => "
                                                "CORE_ERROR_NULLPTR");
                        }
                        ...
                        response.Set1ByteParam(
                                Core::Status::CORE_ERROR_UNKNOWN_CLUSTER);
                        LOG_COMPONENT_ERROR("Communication",
                                            "RX => "
                                            "CORE_ERROR_UNKNOWN_"
                                            "CLUSTER");
        ...
        Core::Status Communication::SendMessage(const Frame &message) {
            const size_t size = Protocol::Encode(
                    message, const_cast<char *>(this->mBufferTx));
            if (size != 0U) {
                this->mSocket.Send(const_cast<const char *>(this->mBufferTx),
                                   size);
            ...
            while (nbData != 0U) {
                ...
                if (rc == '<') {
                    ...
                    if ((rc >= '0' && rc <= '9') || (rc >= 'A' && rc <= 'F')) {
                        if (this->mIndexBufferRx <
                            sizeof(this->mBufferRx) - 1) {
