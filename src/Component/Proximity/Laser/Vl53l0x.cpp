...
Core::CoreStatus Vl53l0x::Initialize ( void )
{
	...
	if ( data == 0xEE )
	{
		...
		if ( this->GetSpadInfo( &spad_count, &spad_type_is_aperture ) )
		{
			...
			this->mI2c.WriteRegisters( this->mAddress, VL53L0X_GLOBAL_CONFIG_SPAD_ENABLES_REF_0, ref_spad_map, 6U );
...
void Vl53l0x::GetSequenceStepTimeouts ( SequenceStepEnables const *enables, SequenceStepTimeouts *timeouts )
{
	...
	this->mI2c.ReadRegister16Bits( this->mAddress, VL53L0X_MSRC_CONFIG_TIMEOUT_MACROP, timeouts->msrc_dss_tcc_mclks );
	...
	this->mI2c.ReadRegister16Bits( this->mAddress, VL53L0X_PRE_RANGE_CONFIG_TIMEOUT_MACROP_HI, timeouts->pre_range_mclks );
