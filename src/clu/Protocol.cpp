...
Protocol::ProtocolStatus Protocol::Decode (const char *frameBuffer, Frame &frame)
{
	...
	if (frameLenght >= 6U && frameLenght % 2U == 0U)
	{
		...
		frame.clusterId = Protocol::ConvertHexCharToInt(frameBuffer[0U]) * 16U + Protocol::ConvertHexCharToInt(frameBuffer[1U]);
		frame.commandId = Protocol::ConvertHexCharToInt(frameBuffer[2U]) * 16U + Protocol::ConvertHexCharToInt(frameBuffer[3U]);
		frame.nbParams  = Protocol::ConvertHexCharToInt(frameBuffer[4U]) * 16U + Protocol::ConvertHexCharToInt(frameBuffer[5U]);
		...
		else if ( (frame.nbParams * 2U) + 6U == frameLenght)
		{
			...
			{
				frame.params[i / 2U] = Protocol::ConvertHexCharToInt(frameBuffer[6 + i]) * 16U + Protocol::ConvertHexCharToInt(frameBuffer[7U + i]);
			}
