...
namespace Misc
{
    namespace Utils
    {
        ...
        template<typename T>
        static inline T Lerp(const T a,
                             const T b,
                             const T t) {
            return a + t * (b - a);
        }
...
#define UINT24_TO_ARRAY(val) \
    {static_cast<uint8_t>(val & 0xFFU), static_cast<uint8_t>(val >> 8U), static_cast<uint8_t>(val >> 16U)}
...
#define PTR_TO_UINT48(arr)                                                                       \
    (uint32_t) (uint64_t) (arr)[0U] | (uint64_t) (arr)[1U] << 8U | (uint64_t) (arr)[2U] << 16U | \
            (uint64_t) (arr)[3U] << 24U | (uint64_t) (arr)[4U] << 32U | (uint64_t) (arr)[5U] << 40U;
