...
namespace Component {
Ssd1306::Ssd1306( TwiInterface &twi, const uint8_t address )
	: mTwi( twi )
	, mAddress( address )
	, mBufferScreen{ 0x00U }
	, mNeedToUpdate( false )
...
Core::CoreStatus Ssd1306::Initialize ( void )
{
	...
	this->mTwi.WriteRegister( this->mAddress, SSD1306_SETLOWCOLUMN, ( vccstate == SSD1306_EXTERNALVCC ) ? 0x10 : 0x14 );
	...
	this->mTwi.WriteRegister( this->mAddress, SSD1306_SETLOWCOLUMN, ( vccstate == SSD1306_EXTERNALVCC ) ? 0x22 : 0xF1 );
	uint8_t init5[] = {
...
void Ssd1306::Update ( const uint64_t currentTime )
{
	...
	if ( this->mNeedToUpdate == true )
	{
		...
		{
			this->mTwi.WriteRegisters( this->mAddress, SSD1306_SETSTARTLINE, &this->mBufferScreen[BUFFER_DISPLAY_LENGTH - this->mUpdateIndex], NB_BYTES );
...
void Ssd1306::EraseArea ( uint16_t x, uint16_t y, uint16_t width, uint16_t height )
{
	for ( uint8_t loop_x = 0U; loop_x < width; loop_x++ )
	{
		...
		{
			Ssd1306::DrawPixel( x + loop_x, y + loop_y, COLOR_BLACK );
		}
	}
...
void Ssd1306::DrawChar ( char c, uint16_t x, uint16_t y, Color color )
{
	uint8_t i, j;
	...
	const uint8_t *chr = Font::font[(uint8_t) c];
	...
	for ( j = 0U; j < CHARACTER_WIDTH; j++ )
	{
		for ( i = 0U; i < CHARACTER_HEIGHT; i++ )
		{
			...
			{
				Ssd1306::DrawPixel( x + j, y + i, color == COLOR_BLACK ? COLOR_WHITE : COLOR_BLACK );
			}
		}
	}
	this->mNeedToUpdate = true;
...
void Ssd1306::DrawString ( const char *str, uint16_t x, uint16_t y, Color color )
{
	while ( *str )
	{
...
void Ssd1306::DrawBitmap ( const SBitmap *bmp, uint16_t x, uint16_t y, Color color )
{
