...
#define BUFFER_DISPLAY_LENGTH                           SCREEN_WIDTH *( ( SCREEN_HEIGHT + 7U ) / 8U )
	...
	Ssd1306( TwiInterface &twi, const uint8_t address = SSD1306_ADDRESS );
	~Ssd1306() = default;
	...
	virtual void DrawChar( char c, uint16_t x, uint16_t y, Color color ) final override;
	virtual void DrawString( const char *str, uint16_t x, uint16_t y, Color color ) final override;
	virtual void DrawBitmap( const SBitmap *bmp, uint16_t x, uint16_t y, Color color ) final override;
