    ...
    namespace Display
    {
        ...
        public:
            ...
            MOCK_METHOD5(DrawLine,
                         void(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color));

            MOCK_METHOD5(DrawRectangle,
                         void(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color));

            MOCK_METHOD4(DrawCircle, void(uint16_t xc, uint16_t yc, uint16_t r, uint16_t color));

            MOCK_METHOD4(DrawChar,
                         void(char c, uint16_t x, uint16_t y, Bitmap::Bitmaps::Color color));

            MOCK_METHOD4(
                    DrawString,
                    void(const char *str, uint16_t x, uint16_t y, Bitmap::Bitmaps::Color color));

            MOCK_METHOD4(DrawBitmap,
                         void(const Bitmap::Bitmaps::SBitmap *bmp,
                              uint16_t                        x,
                              uint16_t                        y,
