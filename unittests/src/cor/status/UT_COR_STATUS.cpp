...
namespace Core
{
    ...
    TEST_F(UT_COR_STATUS, CORE_OK_Ok) {
        EXPECT_STREQ("CORE_OK",
                     Core::StatusToString(Core::Status::CORE_OK).c_str());
    }

    TEST_F(UT_COR_STATUS, CORE_ERROR_Ok) {
        EXPECT_STREQ("CORE_ERROR",
                     Core::StatusToString(Core::Status::CORE_ERROR).c_str());
    }

    TEST_F(UT_COR_STATUS, CORE_ERROR_ARGUMENT_Ok) {
        EXPECT_STREQ("CORE_ERROR_ARGUMENT",
                     Core::StatusToString(Core::Status::CORE_ERROR_ARGUMENT)
                             .c_str());
    }
    ...
    TEST_F(UT_COR_STATUS, CORE_ERROR_OVERLOAD_Ok) {
        EXPECT_STREQ("CORE_ERROR_OVERLOAD",
                     Core::StatusToString(Core::Status::CORE_ERROR_OVERLOAD)
                             .c_str());
    }
    ...
    TEST_F(UT_COR_STATUS, CORE_ERROR_MIN_Ok) {
        EXPECT_STREQ(
                "CORE_ERROR_MIN",
                Core::StatusToString(Core::Status::CORE_ERROR_MIN).c_str());
    }

    TEST_F(UT_COR_STATUS, CORE_ERROR_MAX_Ok) {
        EXPECT_STREQ(
                "CORE_ERROR_MAX",
                Core::StatusToString(Core::Status::CORE_ERROR_MAX).c_str());
    }
    ...
    TEST_F(UT_COR_STATUS, CORE_ERROR_UNKNOWN_CLUSTER_Ok) {
        EXPECT_STREQ(
                "CORE_ERROR_UNKNOWN_CLUSTER",
                Core::StatusToString(Core::Status::CORE_ERROR_UNKNOWN_CLUSTER)
                        .c_str());
    }

    TEST_F(UT_COR_STATUS, CORE_ERROR_DEFAULT_Ok) {
        EXPECT_STREQ("UNKNOWN_STATUS",
                     Core::StatusToString(Core::Status(-50)).c_str());
    }
    ...
    TEST_F(UT_COR_STATUS, OperatorOr_Ok) {
        ...
        EXPECT_EQ(status,
                  Core::Status::CORE_ERROR | Core::Status::CORE_OK |
                          CORE_ERROR_DISABLE);
    }
} // namespace Core
