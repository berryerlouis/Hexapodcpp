		...
		namespace Laser
		{
			...
			protected:
				...
				virtual void SetUp() {
					...
					EXPECT_CALL(mMockTwi, ReadRegister( _, VL53L0X_IDENTIFICATION_MODEL_ID, _ )).WillOnce(
					...
					EXPECT_CALL(mMockTwi, ReadRegister( _, VL53L0X_RESULT_INTERRUPT_STATUS, _ )).WillRepeatedly(
			...
			TEST_F(UT_CMP_VL53L0X, SetThreshold) {
				EXPECT_EQ(Core::CoreStatus::CORE_OK, mVl53l0x.SetThreshold( 350U ));
				EXPECT_EQ(350U, mVl53l0x.GetThreshold());
			}

			TEST_F(UT_CMP_VL53L0X, UnderThreshold) {
				EXPECT_CALL(mMockTwi, ReadRegister( _, VL53L0X_RESULT_INTERRUPT_STATUS, _ )).WillOnce(
					DoAll(SetArgReferee<2U>(0x07U), Return(true)));
				EXPECT_CALL(mMockTwi, ReadRegister16Bits( _, VL53L0X_RESULT_RANGE_STATUS + 10, _ )).WillOnce(
