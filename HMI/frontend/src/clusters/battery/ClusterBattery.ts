...
export class ClusterBattery {
    ...
    private robot: any;
    private batteryStatusHMI: JQuery<HTMLElement>;
    private batteryVoltageHMI: JQuery<HTMLElement>;
    ...
    initialize() {
        this.messageManager.addCallbackNotifyOnSpecificCommand(ClusterName.BATTERY, CommandBattery.STATUS, (message: any) => {
            if (message.size > 0) {
        ...
        this.messageManager.addCallbackNotifyOnSpecificCommand(ClusterName.BATTERY, CommandBattery.VOLTAGE, (message: any) => {
            if (message.size > 0) {
                let voltage = message.fetchInt16U() / 100;
                this.databaseManager.updateDb({
                    cluster: 'BATTERY',
