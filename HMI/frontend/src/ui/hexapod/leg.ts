...
import {hexapodConfiguration as hexapod, LegConfig } from './confHexapod.js';
...
export default class Leg extends THREE.Group {
    groupcoxa: THREE.Group;
    groupfemur: THREE.Group;
    groupTibia: THREE.Group;

    constructor(name: string, leg: LegConfig) {
        super();
        ...
        this.rotation.y = leg.rotation.y;
        ...
        // Coxa servo
        const coxaServo = new Servo("coxa-" + this.name, { x: 0, y: 0, z: -hexapod.body.servo.height / 4 }, { x: 0, y: 0, z: 0 });
        const femurServo = new Servo("femur-" + this.name, { x: this.position.x > 0 ? 25.4 : -25.4, y: 0, z: -hexapod.body.servo.height / 4 }, { x: THREE.MathUtils.degToRad(-90), y: 0, z: 0 });
        const tibiaServo = new Servo("tibia-" + this.name, { x: this.position.x > 0 ? -90 : 90, y: -40, z: -10 }, { x: 0, y: 0, z: 0 });

        // Femur
        const geometry1 = new THREE.BoxGeometry(110.4, 1.5, 20);
        const material1 = new THREE.MeshBasicMaterial({ color: "darkgray" });
        const femur = new THREE.Mesh(geometry1, material1);
        ...
        // Tibia
        const geometry2 = new THREE.BoxGeometry(125, 1.5, 20);
        const material2 = new THREE.MeshBasicMaterial({ color: "darkgray" });
        const tibia = new THREE.Mesh(geometry2, material2);
    ...
    move(coxa: number, femur: number, tibia: number): void {
    ...
    moveServo(servoId: number, rotation: number): void {
        if (servoId === 0) this.moveCoxa(rotation);
        if (servoId === 1) this.moveFemur(rotation);
        if (servoId === 2) this.moveTibia(rotation);
    }

    moveCoxa(coxa: number): void {
        this.groupcoxa.rotation.y = -THREE.MathUtils.degToRad(coxa);
    }

    moveFemur(femur: number): void {
        this.groupfemur.children[1].rotation.y = THREE.MathUtils.degToRad(180) + THREE.MathUtils.degToRad(femur);
    }

    moveTibia(tibia: number): void {
        this.groupTibia.children[0].rotation.y = THREE.MathUtils.degToRad(-90) + THREE.MathUtils.degToRad(-tibia);
    }
}