...
ClusterName.GENERAL = 'GENERAL';
ClusterName.IMU = 'IMU';
ClusterName.PROXIMITY = 'PROXIMITY';
...
class Clusters {
    static getClusterByNameOrCode(identifier, isCode = false) {
        const clusterFound = Clusters.clusters.find(cluster => isCode ? cluster.code.toLowerCase() === identifier.toLowerCase() : cluster.name.toLowerCase() === identifier.toLowerCase());
        if (!clusterFound) {
...
Clusters.clusters = [
    ...
    new Cluster(ClusterName.PROXIMITY, '02', [
    ...
    new Cluster(ClusterName.SERVO, '03', [
